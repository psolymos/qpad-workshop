...
SPP <- sample(SPP)[1:10]
sapply(yall_dis[SPP], sum)

## number of rows
n <- nrow(yall_dis[[1]])
Ystack <- as.matrix(do.call(rbind, yall_dis[SPP]))
Dstack <- matrix(c(0.5, 1, Inf), nrow(Ystack), 3, byrow=TRUE)
Xstack <- lhreg_data[match(rep(SPP, each=n), lhreg_data$spp),]
dim(Ystack)

## Constant
Mmulti <- cmulti(Ystack | Dstack ~ spp - 1, Xstack, type="dis")

Mmulti <- cmulti(Ystack | Dstack ~ logmass + MaxFreqkHz + Mig2 + Hab2, 
                 Xstack, type="dis")
```
